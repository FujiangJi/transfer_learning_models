...
import datetime
from Models import GPR_model, PLSR_model
import warnings
import multiprocessing as mp
import psutil
warnings.filterwarnings('ignore')

def GPR_PLSR_DNN_spatial_CV(tr, tr_obs, refl_obs,n_splits,n_iterations):
    X = refl_obs
    y = tr_obs
    ...
    df_within.to_csv(f'/scratch/fji7/transfer_learning_paper/3_results/{tr}/5_{tr}_spatial CV train GPR_PLSR_df_within.csv',index = False)
    df_out.to_csv(f'/scratch/fji7/transfer_learning_paper/3_results/{tr}/5_{tr}_spatial CV train GPR_PLSR_df_out.csv',index = False)
    vip_score.to_csv(f'/scratch/fji7/transfer_learning_paper/3_results/{tr}/5_{tr}_spatial CV train GPR_PLSR_VIP.csv',index = False)
    plsr_coef.to_csv(f'/scratch/fji7/transfer_learning_paper/3_results/{tr}/5_{tr}_spatial CV train GPR_PLSR_coefficients.csv',index = False)
    return
...
data.dropna(subset=["LMA"],inplace = True)
tr_obs, refl_obs = data.loc[:,'Dataset ID':], data.loc[:,'450':'2400']
tr_obs["LMA"] = tr_obs["LMA"].astype(float)
refl_obs = refl_obs.astype(float)

refl_obs.dropna(inplace = True)
tr_obs = tr_obs.iloc[refl_obs.index]
...
GPR_PLSR_DNN_spatial_CV(tr, tr_obs, refl_obs, n_splits, 10)
