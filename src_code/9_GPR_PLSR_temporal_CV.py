...
def GPR_PLSR_temporal_CV(tr, tr_obs, refl_obs,n_splits,n_iterations):
    X = refl_obs
    y = tr_obs
    ...
    df_within.to_csv(f'/scratch/fji7/transfer_learning_paper/3_results/{tr}/9_{tr}_temporal CV train GPR_PLSR_df_within.csv',index = False)   
    df_out.to_csv(f'/scratch/fji7/transfer_learning_paper/3_results/{tr}/9_{tr}_temporal CV train GPR_PLSR_df_out.csv',index = False)
    vip_score.to_csv(f'/scratch/fji7/transfer_learning_paper/3_results/{tr}/9_{tr}_temporal CV train GPR_PLSR_VIP.csv',index = False)
    plsr_coef.to_csv(f'/scratch/fji7/transfer_learning_paper/3_results/{tr}/9_{tr}_temporal CV train GPR_PLSR_coefficients.csv',index = False)
    return
...
for tr in tr_name:
    data = pd.read_csv(f"/scratch/fji7/transfer_learning_paper/0_datasets/{tr}_dataset_temporal.csv")
    tr_obs, refl_obs = data.loc[:,'Dataset ID':], data.loc[:,'450':'2400']
    
    p = pool.apply_async(func=GPR_PLSR_temporal_CV,args= (tr, tr_obs, refl_obs, n_splits, 10))
